      ...
      >
        ...
        <div ref={navRef} className="sticky top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-[#E5E7EB] shadow-sm transition-all duration-300 gpu-layer">
          ...
          <motion.div
            ref={stripRef}
            layoutScroll
            className="flex gap-2 px-4 pb-2 overflow-x-auto scrollbar-hide rail rail-fade scroll-px-4"
          >
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    "relative isolate shrink-0 flex items-center gap-1 rounded-full border",
                    // Roomier than the old px-3/py-1.5 (~28px tall) — these are
                    // the primary navigation on a phone and were fiddly to hit.
                    "px-3.5 py-2 text-[13px] font-semibold",
                    // Named properties, not transition-all: the sliding pill is
                    // layout-animated by framer, and a blanket transition made
                    // the button animate against it.
                    "transition-[color,background-color,border-color,transform] duration-200 active:scale-[0.96]",
                    active
            ...
            filteredByCat.map(({ cat, items: catItems }, idx) => {
              ...
              return (
                <div key={cat.id} id={`cat-${cat.id}`} className="cv-section" style={{ scrollMarginTop: navH }}>
                  {/* Category header */}
                  <div
                    // Solid white, not blur: one blurred layer per category
                    // header stacked up behind the nav's own blur for no visual
                    // gain — bg-white/85 over white content already read opaque.
      ...
      <div className={`fixed inset-0 z-40 mx-auto w-full ${FRAME} pointer-events-none gpu-layer`}>
        ...
        {categories.length > 1 && (
          <motion.button
            ...
            className={[
              "pointer-events-auto absolute left-1/2 -translate-x-1/2 bg-[#1C1C2E] text-white rounded-2xl px-4 py-3 shadow-xl flex items-center gap-2",
        ...
        <AnimatePresence>
          {activeOrder && !statusOpen && cartCount === 0 && (
            ...
            >
              <div>
                <p className="text-white text-sm font-semibold tracking-wide">Your order · Table {activeOrder.table}</p>
                <p className="text-white/80 text-xs mt-0.5">Tap to view · add more</p>
              </div>
              <span className="text-white font-bold text-sm bg-white/20 px-3 py-1.5 rounded-full">View →</span>
            </motion.button>
          )}
        </AnimatePresence>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={onClose}>
      ...
      <motion.div
        initial={{ y: 60 }}
        animate={{ y: 0 }}
        className={`relative bg-white rounded-t-3xl w-full ${FRAME} p-5 pb-8 max-h-[70vh] flex flex-col`}
